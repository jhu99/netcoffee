...
SimulatedAnnealing<RS,ST,NP,OP>::initial_t(RS& recordstore,NP& np)
{
  ...
  std::cout <<"The number of matching edges is :"<<recordstore.matchingEdges->length <<std::endl;/// The number is 82105.
  return true;
}
...
//bool
//SimulatedAnnealing<RS,ST,NP>::run(RS& recordstore, NP& np)
//{
  //unsigned k=0;
  //initial(recordstore,np);
  //float t=_tmax;
  //float step=(_tmax-_tmin)/_Kmax;
  //std::vector<typename Solution_X_Type::Node> overlappednodes;
  //std::vector<unsigned> overlappedids;
  ...
  //std::default_random_engine generator(seed);
  //std::uniform_real_distribution<float> distribution(0.0,1.0);
  //std::uniform_int_distribution<unsigned> sample_distribution(0,MAXIMUM_RECORDS-1);
  //while(k<_Kmax)
  //{
    //t=t-step;
    //float beta=1.0/(_k*t);
    ...
    //{
      //unsigned select=sample_distribution(generator);
      //if(!_x.validRecord(select,recordstore,np))
      //{
        //--n;
        //continue;
      //}    
      //if(!_x.isOverlapped(select,recordstore))
      //{
        //_x.add(select,recordstore,np);
      //}
      //else
      //{
        //overlappednodes.clear();
        //overlappedids.clear();
        //float delta;
        //delta=_x.slow_discrepancy(select,recordstore,np,overlappednodes,overlappedids);
        //if(g_verbosity>VERBOSE_ESSENTIAL)
        //{
          ////std::cerr << exp(beta*delta)<<std::endl;
        //}
        ...
        //else
        //{
          //_x.recoverFromNew(select,recordstore,np,overlappedids);
        //}
      //}
    //}
    ////std::cerr <<"Step "<<k<<"/"<<_Kmax<<"..."<< std::endl;
  ...
  //std::cerr <<_x.reCalculateScore(recordstore)<< std::endl;
  //return true;
//}
...
SimulatedAnnealing<RS,ST,NP,OP>::printAlignment_t(std::string& filename, NetworkPool_Type& networkpool)
{
  ...
  for ( NodeIt it(_x.getGraph()); it!=lemon::INVALID; ++it)
  {
    ...
    for(i=0;i<_numSpecies;i++)
    {
      _x.getNodeLabel(it,i,matchset);
    }
    if(matchset.size()>=2)
    {
      unsigned j=0;
      for(j=0;j<matchset.size()-1;j++)
      {
        outfile1 << matchset[j] << "\t";
      }
      outfile1 << matchset[j] << std::endl;
    }
